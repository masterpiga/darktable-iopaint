...
to inpaint/clean up selected images and bring the results back into your library, with
rating, color labels and tags copied from the originals.
...
1. Clone this fork and run the setup script. It creates a virtualenv (`.venv` in the
   ...
   ```bash
   git clone <your-fork-url> IOPaint
   cd IOPaint
   ./darktable/setup.sh
   ```
...
2. Set the **source checkout** preference to the path of this clone. That's the only launch
   setting — the script runs the server from the checkout's `.venv` automatically. (`setup.sh`
   already built the frontend; the script will rebuild it on first launch only if it's
   missing, e.g. if you skipped `setup.sh`.)

## Install the script

Either:

- Load it with darktable's **script_manager**, or
- Copy or symlink `darktable/iopaint.lua` into `~/.config/darktable/lua/` and add this line to
...
Restart darktable (or restart Lua via script_manager).
...
| **source checkout (this fork)** | *(empty)* | Path to this cloned fork. **Set this** — it's the only launch setting. The server is run from the checkout's `.venv` (created by `setup.sh`); the frontend, also built by `setup.sh`, is rebuilt on first launch only if missing. |
